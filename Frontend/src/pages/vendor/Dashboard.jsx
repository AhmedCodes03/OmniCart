...
export default function VendorDashboard() {
  ...
  const salesData = {
    labels: stats.sales_history?.length > 0 
      ? stats.sales_history.map(h => h.day.split('-').slice(1).join('/')) 
      : ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    datasets: [
      {
        label: 'Daily Revenue',
        data: stats.sales_history?.length > 0 
          ? stats.sales_history.map(h => h.revenue) 
          : [0, 0, 0, 0, 0, 0, 0],
